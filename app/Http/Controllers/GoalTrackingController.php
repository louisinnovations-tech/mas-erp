...
class GoalTrackingController extends Controller
{
    ...
    public function index()
    {
            ...
            $user = \Auth::user();
            if($user->type == 'employee' || $user->type == 'advocate')
            {
                $employee      = Employee::where('user_id', $user->id)->first();
                if($employee){
                    $goalTrackings = GoalTracking::where('created_by', '=', \Auth::user()->creatorId())->where('branch', $employee->branch_id)->with(['branches','goalType'])->get();
                } else {
                    $goalTrackings = GoalTracking::where('created_by', '=', \Auth::user()->creatorId())->with(['branches','goalType'])->get();
                }
            }
            else
            {
