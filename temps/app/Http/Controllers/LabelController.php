...
class LabelController extends Controller
{
    ...
    public function destroy(Label $label)
    {
        if(\Auth::user()->can('manage crm'))
        {
            ...
            // $leadData = Lead::where('labels', $label->id)->first();
            // $dealData = Deal::where('labels', $label->id)->first();
            // if(!empty($leadData) || !empty($dealData))
            // {
            //     return redirect()->back()->with('error', __('this label is already use so please transfer or delete this label related data.'));
            // }
